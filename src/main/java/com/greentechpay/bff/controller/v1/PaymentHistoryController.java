...
public class PaymentHistoryController {
    ...
    @GetMapping("/page")
    public ResponseEntity<PageResponse<LocalDate, List<ResponsePaymentHistoryDto>>>
    getAllWithPageByUserId(@RequestParam @NotBlank(message = "user id can not be empty") String userId,
        ...
        return ResponseEntity.ok(paymentHistoryService.getAllByPage(userId, page, size));
    }

    @GetMapping("/sender-request-id/{id}")
    public ResponseEntity<ReceiptDto>
    getSenderRequestId(@PathVariable String id) {
...
//TODO write circuit breaker
    ...
    getStatisticsByUserId(@RequestParam @NotBlank(message = "user id can not be empty") String userId,
                          @RequestParam @Past(message = "date must be a past date") LocalDate startDate,
                          @RequestParam @Past(message = "date must be a past date") LocalDate endDate) {
